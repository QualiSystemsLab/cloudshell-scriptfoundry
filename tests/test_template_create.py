...
def test_local_create(local_templates_path, output_dir):
    try:
        template_handler.create_from_local(local_templates_path=local_templates_path,
...
def test_github_create(github_repo, output_dir):
    try:
        template_handler.create_from_github(github_repo=github_repo,
                                            script_name="setup github test",
                                            script_type="setup",
                                            output_dir=output_dir)
    except OutputDirExistsException:
        pass
...
def test_invalid_script_name(local_templates_path, output_dir):
    with pytest.raises(template_handler.InvalidScriptType):
        template_handler.create_from_local(local_templates_path,
                                           script_name="setup pytest invalid",
                                           script_type="lol",
                                           output_dir=output_dir)
